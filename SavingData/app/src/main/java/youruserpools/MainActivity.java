...
public class MainActivity extends AppCompatActivity {
            ...
            case 1:
                ...
                if(resultCode == RESULT_OK) {
                    ...
                    if (!name.isEmpty() && !userPasswd.isEmpty()) {
                        ...
                        AppHelper.getPool().getUser().getDetails(new GetDetailsHandler() {
                            @Override
    ...
    AuthenticationHandler authenticationHandler = new AuthenticationHandler() {
        @Override //here we get the token
        public void onSuccess(CognitoUserSession cognitoUserSession, CognitoDevice device) {
            cognitoUserSession.getIdToken();
            //AppHelper.getCurrSession().getIdToken().getJWTToken().toString(); // add to rest cl
            Log.d(TAG, " -- Auth Success "+ cognitoUserSession.getIdToken().getJWTToken().toString());
            AppHelper.setCurrSession(cognitoUserSession);
            CheckApiConnectionUtil.checkIFServerResponse(getApplicationContext());//todo move to
