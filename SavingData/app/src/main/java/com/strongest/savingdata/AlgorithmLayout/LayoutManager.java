...
//        updateLayoutStats(haveHolder);
    ...
    public void drawBody(ArrayList<PLObjects> toLayout, Muscle muscle) {
        ++numOfBodyParts;

        toLayout.add(new PLObjects.BodyText(muscle, numOfWorkouts - 1, numOfBodyParts, null));
    }

    public void drawExercise(ArrayList<PLObjects> toLayout, BeansHolder beansHolder, Muscle muscle) {
        ++numOfExercises;
        ...
        list.add(beansHolder);
        list.add(beansHolder);
        PLObjects.ExerciseProfile ep = new PLObjects.ExerciseProfile(list, muscle, numOfWorkouts - 1, numOfBodyParts, numOfExercises);
  ...
  /*  public void updateRep(int position, Beans rep) {
    ...
    private void drawBodyFromTemplate(int workoutPosition) {
        for (int i = 0; i < programTemplate.getBodyPartsPerWorkout(workoutPosition); i++) {
            ...
            drawBody(layout, m);
            // programTemplate.setBodyPart(m);
            drawExerciseFromTemplate(m);
        }
    }

    private void drawExerciseFromTemplate(Muscle m) {

        for (int i = 0; i < programTemplate.getBlockLength(m); i++) {
            drawExercise(layout, null, programTemplate.getMuscle());
        }
    }
            ...
            do {
                ...
                switch (c.getInt(c.getColumnIndex(TYPE))) {
                    ...
                    case 2:
                        ...
                        drawExercise(layout, beansHolder, muscle);
   ...
   /* private void setWorkoutTemplate() {
        MainMuscleEnum[][] templateArray = matrixListtoArray(bodyTemplate);
        programTemplate = new ProgramTemplate(context, templateArray);
        ...
        if (updateComponents != null) {
            ...
            switch (updateComponents.getUpdateType()) {
                case Swap:
                    ...
                    break;
                case Insert:
                    getSplitRecyclerWorkouts().get(workoutPosition).add(updateComponents.getInsertPosition(), updateComponents.getExerciseProfile());
                    break;
                case Remove:
   ...
   /* private MainMuscleEnum[][] matrixListtoArray(ArrayList<ArrayList<MainMuscleEnum>> arr) {
    ...
    public ArrayList<ArrayList<PLObjects>> getSplitRecyclerWorkouts() {
        if (splitRecyclerWorkouts == null) {
            splitRecyclerWorkouts = new ArrayList<>();
            int j = 1;
            for (int i = 0; i < getNumOfWorkouts(); i++) {
                splitRecyclerWorkouts.add(new ArrayList<PLObjects>());
                if (layout.size() == 0) {
                ...
                while (j < layout.size()) {
                    if (layout.get(j).getType() == WorkoutLayoutTypes.WorkoutView) {
                        ...
                        splitRecyclerWorkouts.get(i).add(layout.get(j));
                    }
                    j++;
                }
    ...
    public void requestSplitToLayout() {
        ...
        for (int i = 0; i < arr.size(); i++) {
            drawWorkout(layout, programTemplate.getWorkoutsNames().get(i));
    ...
    public int getNumOfExercises() {
        if (numOfExercises != 0) {
            int count = 0;
            for (int i = 0; i < layout.size(); i++) {
                if (layout.get(i).getType() == WorkoutLayoutTypes.ExerciseView) {
                    count++;
                }
            }
    ...
    public ArrayList<ArrayList<PLObjects>> getMusclesPerWorkout() {
        ArrayList<ArrayList<PLObjects>> arr = new ArrayList<>();
        int j = -1;
        for (int i = 0; i < layout.size(); i++) {
            if (layout.get(i).getType() == WorkoutLayoutTypes.WorkoutView) {
...
public static class UpdateComponents {
    ...
    public UpdateComponents(LayoutManager.Update updateType) {
    ...
    public PLObjects.ExerciseProfile getExerciseProfile() {
