 ...
 */
public class LoginActivity2 extends BaseActivity implements LoaderCallbacks<Cursor> {
  ...
  /*  private boolean mayRequestContacts() {
    ...
    private boolean vaildate() {
        ...
        if (email == null || email.isEmpty()) {
            valid = false;
            mEmailView.setError("You must enter email address");
        }
        if (pass == null || pass.isEmpty()) {
            mPasswordView.setError("You must enter password");
            valid = false;
        }
        if (!validEmail(email)) {
            mEmailView.setError("Email address is incorrect");
            valid = false;
        }
        return valid;
    }
